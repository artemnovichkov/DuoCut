...
- [x] Иконка, AccentColor, гаптика, анимации переходов, пустые состояния. Звука нет — отдельный пункт на будущее.
...
- [ ] Звук (рез, промах, комбо).
- [ ] Видео/гифка для README и соцсетей.
- [ ] Level select внутри пака (сейчас пак идёт подряд с первого уровня).
