...
- [ ] `Polygon`: `[CGPoint]`, `area` (shoelace, знак нормализуем), `centroid`, `boundingBox`, `path`, `applying(translation:rotation:)`, `contains(_:)`.
- [ ] `PolygonCut.split(_:by:)`: signed distance по вершинам, вставка точек пересечения, обход контура с разделением на цепочки → массивы полигонов по каждую сторону. Обязательно корректно для **вогнутых** фигур (несколько пересечений) и для случаев «линия не задевает / касается вершины».
- [ ] Тесты: квадрат пополам (50/50), по диагонали, круг-аппроксимация, L-образная (вогнутая, 2 пересечения), C-образная (4 пересечения → 3 куска), линия мимо фигуры, линия через вершину, вырожденные и почти-параллельные случаи, сумма площадей == площади оригинала (с допуском).
- Готово, когда: тесты зелёные, никакого UI-кода в `Geometry/`.

### Фаза 3 — Нож на шарнире (ядро ощущения)
- [ ] `FoldLine.from(proxy:)`: `proxy.reservedRegions(kind: .division, options: [.includeInactive]).first`; ось — по соотношению сторон рамки (вертикальный / горизонтальный фолд, как в `AvoidDivisionExample.swift`); fallback без шарнира — вертикальная линия по центру контейнера. Читать **внутри** `GeometryReader`, не кэшировать.
- [ ] `CutDetector` (по образцу `DuoBird/Game/FlapDetector.swift`): ловит **резкий хлопок** — сгиб на 10–15° от локального максимума. Отдаёт момент коммита и скорость (°/с) для оценки «чистоты» реза; rearm — новый рез только после разгибания назад на ~5°. Плавный `progress` по текущему углу — только для подсветки лезвия перед хлопком.
- [ ] `BladeOverlay`: линия/лезвие ровно на фолде, входит в фигуру по `progress`, свечение + `sensoryFeedback(.impact)` на коммите.
- [ ] `Piece`: после реза — разлёт половинок (гравитация, импульс от ножа в стороны, вращение), шаг в `TimelineView(.animation)`.
- [ ] Fallback-ввод: свайп поперёк линии реза режет так же — игра остаётся играбельной на обычном iPhone.
...
### Фаза 4 — Puzzle-режим
- [ ] `Level`: полигон(ы), стартовый трансформ, токены (звёзды/цветные точки), `cutsAllowed`, цель:
  `.equalHalves`, `.ratio(Double)`, `.separateColors`, `.tokensPerSide(Int)`.
- [ ] Ввод: drag одним пальцем — перенос, rotation gesture двумя — поворот (`simultaneousGesture`); лёгкий снап к «красивым» углам, но без магнита к правильному ответу.
- [ ] Live-HUD: проценты площадей пересчитываются **до** реза, по текущему положению фигуры относительно линии.
- [ ] Оценка: ⭐⭐⭐ по |Δ| (3★ < 1%, 2★ < 3%, 1★ < 6%), результат-карточка, «следующий уровень».
- [ ] Мульти-рез: последующие резы применяются к уже полученным кускам.
- [ ] Book pose: когда division активна, цель/HUD уходят на верхнюю половину, игровая зона — на нижнюю (через regions, ничего не попадает в фолд).
- [ ] Прогресс (звёзды по уровням) в `UserDefaults`, `Codable`.
- [ ] Пак №1 «Basics»: 8–10 уровней от квадрата к вогнутым фигурам.
- Готово, когда: пак проходится целиком, звёзды сохраняются между запусками.

### Фаза 5 — Arcade-режим
- [ ] Фрукты/фигуры летят по параболам через экран; хлопок сгибом режет всё, что в этот момент пересекает линию фолда.
- [ ] Комбо за несколько объектов в один хлопок, бомбы = конец игры, пропуск = жизнь (3 жизни).
- [ ] Нарастающая сложность, счёт + рекорд в `UserDefaults`, разлёт половинок на общей физике из Фазы 3.
- Готово, когда: режим играется от начала до game over, рекорд сохраняется.

### Фаза 6 — Мета: паки, ачивки, шара
- [ ] Паки №2–4: «Fruit» (вогнутые + токены), «Constellations» (`tokensPerSide`), «Mosaic» (мульти-рез, `ratio`). Прогрессия сложности внутри пака, пак открывается по звёздам.
- [ ] `Achievements`: каталог + `UserDefaults`, тост при разблокировке, экран списка. Например: первый идеальный рез (ровно 50.0%), 10 идеальных, комбо ×5 в аркаде, пак на все звёзды, «Hinge Warranty Voided» на 1000 сгибов.
- [ ] `FoldCounter`: глобальный счётчик сгибов, показывается на экране результатов (самоирония про убитые шарниры).
- [ ] Дейли-челлендж: фигура дня из seed по дате (одна и та же у всех), один рез, результат в процентах, стрик и история последних дней в `UserDefaults`, повтор за день запрещён.
- [ ] `ShareCard` через `ImageRenderer` + `ShareLink`: фигура, разрез, процент, название уровня / номер дня и стрик.
...
- [ ] Иконка, AccentColor, звук/гаптика, анимации переходов, пустые состояния.
- [ ] Скриншоты/гифки: `.github/images/` (`-folded`, `-book` суффиксы для состояний сгиба).
- [ ] `README.md` в стиле DuoBird: что это, требования, геймплей, какие API использованы, «Good to Know» про division region и её активность только при частичном сгибе.
