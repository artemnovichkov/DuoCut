...
### Фаза 8 — ревью верстки и механик (сделано)

Прогон всех экранов на симуляторе в трёх состояниях (плоский, book-поза, сложенный) + разбор механик.

Верстка:
- [x] Fold-aware раскладка для **вертикального** division (раньше сдвигался только горизонтальный):
      заголовок, подсказка, результат-карточка, баннер аркады и тост ачивки уходят в половину.
- [x] `FoldColumns`: паки и ачивки — две колонки по половинам, сгиб как жёлоб, заголовок экрана,
      отступ под кнопку «назад».
- [x] Полоса division рисуется под фигурой (`FoldBand`), а не поверх — фигура больше не грязная.
- [x] Лезвие заметно в покое (2.5pt, 0.5 alpha) + засечки по краям, чтобы было по чему целиться.
- [x] Меню: на широком внутреннем дисплее заголовок в одной половине, режимы в другой.
- [x] Тёмная тема: цвета переехали в ассет-каталог с dark-вариантами.

Механики:
- [x] `startOffset` в юнитах фигуры + принудительный зазор от лезвия: уровень не открывается решённым,
      сложность одинаковая на внутреннем и внешнем дисплее. Масштаб фигуры влезает в половину.
- [x] Мульти-рез судится по финальным кускам (`Goal.shares`), а не по каждому резу; пак Mosaic
      переписан (thirds / quarters / uneven / elbow / three ways).
- [x] Видно, что рез состоялся: куски разных оттенков со швом + точки «сколько резов осталось»,
      финальный рез разбрасывает куски и в мульти-уровнях.
- [x] Промах лезвия сообщается («The blade missed»), первый запуск подсказывает управление.
- [x] Токены улетают вместе с куском, на котором лежали.
- [x] Частичный зачёт в token-уровнях (один токен не на той стороне = 1★).
- [x] `perfectCuts` считается по той же зеркальной ошибке, что и звёзды.
- [x] Звёзды, паки и стрик восстанавливаются при старте — ачивки за них больше не ждут новой сессии.
- [x] Аркада: спавн пачками и разброс момента пересечения (комбо стали достижимы), размер фигур от
      размера поля, прицел по реальной линии сгиба (в т.ч. горизонтальной), 4 секунды без штрафа за
      пропуск, пауза перед рестартом, счёт банкуется при выходе.
- [x] Дейли: день считается один раз — экран результата со стриком и историей недели вместо
      переигровки; фигуры дня асимметричные, у симметричных цель — доля, а не половина.
- [x] Share-карточка рендерится один раз в `.task`, а не на каждом проходе body.
- [x] Свайп-рез требует 60pt и не срывается при отмене драга; фигуру нельзя утащить за край.
- [x] Ordinals в целях («a third», не «3th»), подписи результата по числу звёзд.
...
- [ ] Dynamic Type: шрифты фиксированные (`.system(size:)`) — игровое поле от этого не зависит,
      но списки и карточки могли бы масштабироваться.
